public class Tester {
	...
	public static void testPart3() {
		...
		// Creating a new door and setting a name
		Door door = new Door();
		door.setRoomName("Research Labs");

		// Creating a new lock and attaching it to the door
		CardLock lock = new CardLock();
		door.attachLock(lock);

		// Creating 2 cards: 1 student and 1 staff
		SmartCard cardA = new SmartCard("Anna Undergrad");
		SmartCard cardB = new SmartCard("Dr. Bob Lecturer");
		cardB.setStaff(true);

		// Trying to open the door with the student card
		lock.swipeCard(cardA);
		door.openDoor();

		// Trying to open the door with the staff card
		lock.swipeCard(cardB);
		door.openDoor();
	}
}
